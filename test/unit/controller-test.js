...
contract('Controller Test', function (accounts) {
    ...
    describe('Ownership', function () {
        it('Should only be possible for owner to change ownership', async () => {
            const owner = await controller.owner()
            assert.strictEqual(owner, governance);
            await expect(
                controller.prepareTransferOwnership(investor1, {from:investor1})
            ).to.eventually.be.rejected;
            await expect(
                controller.prepareTransferOwnership(investor1, {from:owner})
            ).to.eventually.be.fulfilled;
            await expect(
                controller.transferOwnership(investor1, {from: owner})
            ).to.eventually.be.fulfilled;
            const newOwner = await controller.owner()
            assert.strictEqual(newOwner, investor1);
            return expect(controller.prepareTransferOwnership(owner, {from:newOwner})).to.eventually.be.fulfilled;
        })

        it('Should not be possible to change ownership unless there is a pending ownerShipChange', async () => {
            await expect(controller.pendingOwner()).to.eventually.be.equal(ZERO);
            await expect(controller.owner()).to.eventually.be.equal(governance);
            return expect(controller.transferOwnership({from:governance})).to.eventually.be.rejected;
        })

        it('Should only be possible for owner to retire the contract', async () => {
            await expect(controller.prepareRenounceOwnership(true, {from: investor1})).to.eventually.be.rejected;
            await expect(controller.prepareRenounceOwnership(false, {from: investor1})).to.eventually.be.rejected;
            await expect(controller.renounceOwnership({from: investor1})).to.eventually.be.rejected;
            await expect(controller.prepareRenounceOwnership(true, {from: governance})).to.eventually.be.fulfilled;
            return expect(controller.renounceOwnership({from: governance})).to.eventually.be.fulfilled;
        })

        it('Should not be possible to recover ownership of a retired contract', async () => {
            await expect(controller.owner()).to.eventually.be.equal(governance);
            await expect(controller.prepareRenounceOwnership(true, {from: governance})).to.eventually.be.fulfilled;
            await expect(controller.renounceOwnership({from: governance})).to.eventually.be.fulfilled;
            await expect(controller.renounceOwnership({from: governance})).to.eventually.be.rejected;
            return expect(controller.owner()).to.eventually.be.equal(ZERO);
        })

        it('Should not be possible to retire contract without prep', async () => {
            await expect(controller.owner()).to.eventually.be.equal(governance);
            return expect(controller.renounceOwnership({from: governance})).to.eventually.be.rejected;
        })
    })
})
