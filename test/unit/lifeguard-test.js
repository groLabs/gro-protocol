...
contract('lifeguard test', function (accounts) {
	//chain
	const chainlinkPrices = ['100028600', '100020884', '100079294'];
	...
	beforeEach('Setup curveOracle and mocks before tests', async function () {
		...
		daiAmount = new BN('463638549121886635068696498');
		usdcAmount = new BN('926899277715061');
		usdtAmount = new BN('681290904719677');
		...
		await buoy.setOracleTolerance('1000');
	...
	describe('Buoy floats', function () {
		it('Should be possible to set a new limit for the safety check', async function () {
			await expect(buoy.setCurveTolerance('200')).to.be.fulfilled;
			return expect(buoy.curve_check_tolerance()).to.eventually.be.a.bignumber.equal(new BN('200'));
		});

		it('Should be possible to set a new limit for the oracle check', async function () {
			await expect(buoy.setOracleTolerance('5000')).to.be.fulfilled;
			return expect(buoy.oracle_check_tolerance()).to.eventually.be.a.bignumber.equal(new BN('5000'));
		});

		it('Should return true if the safety check runs within given paramaters', async function () {
            await buoy.updateRatios();
			return expect(buoy.safetyCheck()).to.be.eventually.be.true;
		});
		...
		it.skip('Should correctly update ratios', async function () {
            // pre update
            await expect(buoy.lastRatio(0)).to.eventually.be.a.bignumber.equal(toBN(0));
            await expect(buoy.lastRatio(1)).to.eventually.be.a.bignumber.equal(toBN(0));
            await expect(buoy.lastRatio(2)).to.eventually.be.a.bignumber.equal(toBN(0));
			await buoy.updateRatios();
            // post update
            await expect(buoy.lastRatio(0)).to.eventually.be.a.bignumber.gt(toBN(0));
            await expect(buoy.lastRatio(1)).to.eventually.be.a.bignumber.gt(toBN(0));
            await expect(buoy.lastRatio(2)).to.eventually.be.a.bignumber.gt(toBN(0));
		});

		it('Should not update the ratios if chainlinks ratios has deviated more than the threshold', async function () {
            // post update
			await buoy.updateRatios();
            await expect(buoy.lastRatio(0)).to.eventually.be.a.bignumber.gt(toBN(0));
            await expect(buoy.lastRatio(1)).to.eventually.be.a.bignumber.gt(toBN(0));
            await expect(buoy.lastRatio(2)).to.eventually.be.a.bignumber.gt(toBN(0));
            const ratio_Dai_Usdc = await buoy.lastRatio(0);
            const ratio_Dai_Usdt = await buoy.lastRatio(1);
            const ratio_Usdt_Usdc = await buoy.lastRatio(2);
            daiAmount = toBN('5000000000').mul(toBN(1E18));
            await mockDAI.mint(accounts[0], daiAmount);
            await mockDAI.approve(curve3Pool.address, daiAmount);
            await curve3Pool.add_liquidity([daiAmount, 0, 0], 0, {
                from: accounts[0],
                gas: '6721975',
                allow_revert: true,
            });
			await buoy.updateRatios();
            await expect(buoy.lastRatio(0)).to.eventually.be.a.bignumber.equal(ratio_Dai_Usdc);
            await expect(buoy.lastRatio(1)).to.eventually.be.a.bignumber.equal(ratio_Dai_Usdt);
            await expect(buoy.lastRatio(2)).to.eventually.be.a.bignumber.equal(ratio_Usdt_Usdc);
        })

		it.skip('Should return false if the safety check runs outside given paramaters', async function () {
			await buoy.setOracleTolerance('0');
			return expect(buoy.safetyCheck()).to.eventually.be.false;
		});

		it('Should be possible to health check the curve pool asset depth', async function () {
            const checkWithinBounds = await buoy.healthCheck(1500)
            const checkOutsideOfBounds = await buoy.healthCheck(2500)
            assert.strictEqual(checkWithinBounds[0], true);
			return assert.strictEqual(checkOutsideOfBounds[0], false);
		});
		...
		it('Should get the correct amount of a single asset from usd', async function () {
			...
			).to.eventually.be.a.bignumber.closeTo('1000000000', toBN(2E6));
		});
	});
	...
	describe('LifeGuard invest', function () {
		...
		it('Should not trigger invest to curve when exceed the threshold', async function () {
			await lifeGuard.setInvestToCurveThreshold(3000);
			const trigger = await lifeGuard.investToCurveVaultTrigger();
